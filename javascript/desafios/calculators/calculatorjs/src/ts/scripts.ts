...
const previousOperationText: HTMLElement = document.querySelector(
  '#previous_operation',
) as HTMLElement;
const currentOperationText: HTMLElement = document.querySelector(
  '#current_operation',
) as HTMLElement;
...
class Calculator {
  ...
  addDigit(digit: string): void {
    ...
    if (
      digit === '.' &&
      this.currentOperationText?.textContent?.includes('.')
    ) {
      return;
    }
  ...
  processOperation(operator: string) {
    ...
    let previous: number = +(
      this.previousOperationText.textContent as string
    ).split(' ')[0];
  ...
  changeOperation(operation: string) {
    ...
    this.previousOperationText.textContent =
      (this.previousOperationText.textContent as string).slice(0, -1) +
      operation;
  }
  //DEL a digit
  processDelOperator() {
    this.currentOperationText.textContent = (
      this.currentOperationText.textContent as string
    ).slice(0, -1);
  }
